...
def compute_loss_value(opt, poisoned_data, model_ascent):
    ...
    for idx, (img, target,ispoison) in tqdm(enumerate(example_data_loader, start=0)):
        if opt.cuda:
            img = img.cuda()
            target = target.cuda()
...
def isolate_data(opt, poisoned_data, losses_idx):
    ...
    for idx, (img, target,ispoison) in tqdm(enumerate(example_data_loader, start=0)):
        ...
        if idx in perm:
            isolation_examples.append((img, target,ispoison))
            cnt += 1
        else:
            other_examples.append((img, target,ispoison))

    error=0

    for i in isolation_examples:
        if i[2]==0:
            error+=1
    print('isolation_examples has{} clean example in 500 poison example'.format(error))
    # Save data
    if opt.save:
        ...
        isolation_examples = np.asarray(isolation_examples, dtype=object)
        other_examples = np.asarray(other_examples, dtype=object)
...
def train_step(opt, train_loader, model_ascent, optimizer, criterion, epoch):
    ...
    for idx, (img, target,ispoison) in enumerate(train_loader, start=1):
        if opt.cuda:
            img = img.cuda()
            target = target.cuda()
        # 损失计算方式
...
def test(opt, test_clean_loader, test_bad_loader, model_ascent, criterion, epoch):
    ...
    for idx, (img, target,ispoison) in enumerate(test_clean_loader, start=1):
        if opt.cuda:
            img = img.cuda()
            target = target.cuda()
    ...
    for idx, (img, target,ispoison) in enumerate(test_bad_loader, start=1):
        if opt.cuda:
            img = img.cuda()
            target = target.cuda()
    ...
    if epoch < opt.tuning_epochs + 1:
        log_root = opt.log_root + '/ABL_results_tuning_epochs_resnet_LGA.csv'
        test_process.append(
