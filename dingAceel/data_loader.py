...
def get_test_loader(opt):
    ...
    if (opt.dataset == 'CIFAR10'):
        ...
        gtsrb_transform = transforms.Compose([transforms.Resize((32, 32)),])
        testset = torchvision.datasets.ImageFolder(root='./data/'+ 'GTSRB/val_images/',transform=gtsrb_transform)
    else:
        raise Exception('Invalid dataset')
...
def get_backdoor_loader(opt):
    ...
    if (opt.dataset == 'CIFAR10'):
        ...
        gtsrb_transform = transforms.Compose([transforms.Resize((32, 32))])
        trainset = torchvision.datasets.ImageFolder(root='./data/'+ 'GTSRB/Training/',transform=gtsrb_transform)
    else:
        raise Exception('Invalid dataset')
...
class Dataset_npy(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ispoison=self.dataset[index][2]
        if self.transform:
            image = self.transform(image)
        # print(type(image), image.shape)
        return image, label,ispoison
...
class DatasetBD(Dataset):
    ...
    def __getitem__(self, item):
        ...
        ispoison=self.dataset[item][2]
        img = self.transform(img)

        return img, label,ispoison
    ...
    def selectTrigger(self, img, width, height, distance, trig_w, trig_h, triggerType):
        ...
        assert triggerType in ['squareTrigger', 'gridTrigger', 'fourCornerTrigger', 'randomPixelTrigger',
                               'signalTrigger', 'trojanTrigger','BppTrigger','SIGTrigger','BlendTrigger']
        ...
        if triggerType == 'squareTrigger':
            ...
            img = self._BlendTrigger(img, width, height)
        elif triggerType == 'BppTrigger':
            img = np.transpose((img), (2, 0, 1))
            img = self.BPP(img, 8)
            img = np.transpose((img), (1, 2, 0))
        elif triggerType == 'SIGTrigger':
            img = self._SIGTrigger(img, width, height)
        else:
    ...
    def BPP(self,image, squeeze_num):
        channel, h, w = image.shape
        for y in range(h):
            for x in range(w):
                old = image[:, y, x]
                temp = np.empty_like(old).astype(np.float64)
                new = np.round_(old / 255.0 * (squeeze_num - 1), 0, temp) / (squeeze_num - 1) * 255
                error = old - new
                image[:, y, x] = new
                if x + 1 < w:
                    image[:,y, x + 1] = image[:,y, x + 1]+error * 0.4375
                if (y + 1 < h) and (x + 1 < w):
                    image[:, y + 1, x + 1] =image[:, y + 1, x + 1] +error * 0.0625
                if y + 1 < h:
                    image[:, y + 1, x] =image[:, y + 1, x] +error * 0.3125
                if (x - 1 >= 0) and (y + 1 < h):
                    image[:, y + 1, x - 1] =image[:, y + 1, x - 1]+error * 0.1875
        return image
    ...
    def _SIGTrigger(self, img, width, height):
        alpha = 0.2
        # load signal mask
        signal_mask = np.load('trigger/signal_cifar10_mask.npy')
    ...
    def _BlendTrigger(self, img, width, height):
        alpha = 0.2
        mask = np.random.randint(low=0, high=256, size=(width, height), dtype=np.uint8)
        blend_img = (1 - alpha) * img + alpha * mask.reshape((width, height, 1))
        blend_img = np.clip(blend_img.astype('uint8'), 0, 255)
