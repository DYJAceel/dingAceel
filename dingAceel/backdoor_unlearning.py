...
def train_step_finetuing(opt, train_loader, model_ascent, optimizer, criterion, epoch):
    ...
    for idx, (img, target,is_poison) in enumerate(train_loader, start=1):
        if opt.cuda:
            img = img.cuda()
            target = target.cuda()
...
def train_step_unlearning(opt, train_loader, model_ascent, optimizer, criterion, epoch):
    ...
    for idx, (img, target,is_poison) in enumerate(train_loader, start=1):
        ...
        optimizer.zero_grad()
        (-loss).backward()  # Gradient ascent training  反学习 损失函数的变化
        optimizer.step()
...
def test(opt, test_clean_loader, test_bad_loader, model_ascent, criterion, epoch):
    ...
    model_ascent.eval()
    # 测试干净数据集的准确率
    for idx, (img, target,is_poison) in enumerate(test_clean_loader, start=1):
    ...
    top5 = AverageMeter()
# 测试脏数据集的准确率
    for idx, (img, target,is_poison) in enumerate(test_bad_loader, start=1):
        if opt.cuda:
            img = img.cuda()
            target = target.cuda()
    ...
    log_root = opt.log_root + '/ABL_unlearning_LGA.csv'
    test_process.append(
...
def train(opt):
    ...
    model_ascent, checkpoint_epoch = select_model(dataset=opt.dataset,
                                ...
                                pretrained_models_path='./1/ResNet34-tuning_epochs5.tar',
    ...
    data_path_isolation = os.path.join(opt.isolate_data_root, "{}_isolation{}%_examples_resnet_LGA.npy".format(opt.model_name,
                                                                                                    opt.isolation_ratio * 100))
    data_path_other = os.path.join(opt.isolate_data_root, "{}_other{}%_examples_resnet_LGA.npy".format(opt.model_name,
    ...
    if opt.finetuning_ascent_model == True:#微调模型 提升干净识别准确率
        ...
        for epoch in range(0, opt.finetuning_epochs):
            learning_rate_finetuning(optimizer, epoch, opt)#微调学习率调整
            train_step_finetuing(opt, isolate_other_data_loader, model_ascent, optimizer, criterion, #微调训练：  改善干净样本识别率
                             epoch + 1)
            test(opt, test_clean_loader, test_bad_loader, model_ascent, criterion, epoch + 1)#测试微调效果
...
def save_checkpoint(state, epoch, is_best, opt):
    if is_best:
        filepath = os.path.join(opt.unlearning_root, opt.model_name + r'-unlearning_epochs_resnet_{}_LGA.tar'.format(epoch))
